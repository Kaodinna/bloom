...
export default function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [post, setPost] = useState<BPost | null>(null)
          ...
          <div className="bg-card border border-border rounded-3xl p-4">
            <div className="flex items-center gap-2.5 mb-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0"
                ...
                <span className="text-xs font-bold text-white">{getInitials(post.author)}</span>
              </div>
              <div className="flex-1">
                <p className="text-sm font-semibold text-primary">Community Member</p>
                <p className="text-2xs text-muted">{timeAgo(post['Created Date'])}</p>
              </div>
            </div>
            <p className="text-sm text-primary leading-relaxed mb-4">{post.content}</p>
            <div className="flex items-center gap-3 pt-3 border-t border-border">
              ...
              <span className="text-xs text-muted">{comments.length} {comments.length === 1 ? 'reply' : 'replies'}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="px-5">
        ...
        <div className="flex flex-col gap-2.5">
          {comments.length === 0 ? (
            ...
            <div key={c._id} className="bg-card border border-border rounded-2xl p-3.5">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0"
                  style={{ background: GRAD[(i + 1) % GRAD.length] }}>
                  <span className="text-2xs font-bold text-white">{getInitials(c.author)}</span>
                </div>
                <span className="text-xs font-semibold text-primary">
                  {c.author === profile?._id ? 'You' : 'Community Member'}
                </span>
                <span className="text-2xs text-muted ml-auto">{timeAgo(c['Created Date'])}</span>
              </div>
              <p className="text-sm text-primary leading-relaxed pl-9">{c.content}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="fixed bottom-16 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-card border-t border-border px-5 py-3 z-40">
        <div className="flex gap-2.5 items-center">
          ...
          <button onClick={handleReply} disabled={!reply.trim() || sending}
            ...
            <Send size={15} color={reply.trim() && !sending ? '#fff' : '#9A9094'} />
          </button>
        </div>
      </div>
